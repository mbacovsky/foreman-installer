...
module ProvisioningWizard
  ...
  def self.setup_wizard(kafo)
    ...
    preselected_interface = preselect_interface(kafo.param('foreman_plugin_staypuft', 'interface'))
    ...
    net_wizard.entries = [
      ...
      f.boolean(:configure_firewall_ssh, :label => 'Allow ssh in firewall on this machine'),
      f.boolean(:configure_firewall_dns, :label => 'Allow dns in firewall on this machine'),
      f.boolean(:configure_firewall_dhcp, :label => 'Allow dhcp in firewall on this machine'),
      f.boolean(:configure_firewall_tftp, :label => 'Allow tftp in firewall on this machine'),
      f.boolean(:configure_firewall_appache, :label => 'Allow connection to webserver on this machine'),
      f.boolean(:configure_firewall_appache, :puppet => 'Allow connection to puppet on this machine'),
    ...
    # preseed networking from selected interface
    net_wizard.update(interfaces[preselected_interface.to_s])

    # set defaults given on CLI
    attrs = net_wizard.values.keys - [:interface]
    ...
    net_wizard.update(cli_defaults)
  ...
  def self.preselect_interface(preferred_interface)
    # TODO preselected_interface = kafo_param(:interface)
    # - test it is valid, skip interface_wizard if possible
    return preferred_interface.to_sym if interfaces.keys.include? preferred_interface
  ...
  def self.current_system_timezone
  ...
  def self.fix_interface_name(facter_name)
